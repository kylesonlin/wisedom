...
function appRouterInstrumentNavigation(client) {
  routerTransitionHandler = (href, navigationType) => {
    const pathname = new URL(href, react.WINDOW.location.href).pathname;
  ...
  react.WINDOW.addEventListener('popstate', () => {
    if (currentRouterPatchingNavigationSpanRef.current?.isRecording()) {
      currentRouterPatchingNavigationSpanRef.current.updateName(react.WINDOW.location.pathname);
      currentRouterPatchingNavigationSpanRef.current.setAttribute(core.SEMANTIC_ATTRIBUTE_SENTRY_SOURCE, 'url');
    } else {
